...
double neuralNetwork::getSetSSE( std::vector<dataEntry*>& set )
{
	double sse = 0;
	ofstream out;
	out.open("compareOutputAndTargetForASet.csv");
	//for every training input array
	for ( int tp = 0; tp < (int) set.size(); tp++)
	{
		...
		for ( int k = 0; k < nOutput; k++ )
		{	
			double temp = clampOutput( outputNeurons[k] ) ;
			out << temp << "," << set[tp]->target[k] << endl;
			
			//sum all the SSEs together
			sse += pow((outputNeurons[k] - set[tp]->target[k]), 2);
		}
   ...
   out.close();
	//calculate error and return as percentage
	return (0.5 * sse)/(nOutput * set.size());
}
...
* Output Clamping
********************************************************************/
inline double neuralNetwork::clampOutput( double x )
{
	//return x;
	if ( x < 0.1 ) return 0;
